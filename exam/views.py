    ...
    if request.method == 'POST':
        ...
        if question_form.is_valid() and option_formset.is_valid():
            ...
            question = Question.objects.create(
                ...
                answer=Option.objects.get(value=request.POST.get('correct_answer'))
            )

            # Create and save the options
            for form in option_formset:
                Option.objects.create(question=question, value=form.cleaned_data['value'])

            return JsonResponse({'success': True, 'message': 'Question saved successfully'})

        else:
            # If there are errors, return a JSON response with form errors
            errors = {'question_errors': question_form.errors, 'option_errors': option_formset.errors}
            print(errors)  # This will print errors to your Django development server console
            return JsonResponse({'success': False, 'errors': errors})

    return render(request, 'exam/admin_add_question.html', {'questionForm': question_form, 'optionFormset': option_formset})
